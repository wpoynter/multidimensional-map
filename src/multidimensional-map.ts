...
function getArrayIntersection<T>(...sets: T[][]) {
    ...
    sets.forEach((set) => {
        set.forEach((item) => {
            if (!numOfAppearances.has(item)) numOfAppearances.set(item, 0)
...
class MultidimensionalMap<EntryT> {
    ...
    constructor(
        ...
        this.dimensions = dimensions.reduce(
            (acc: DimensionCollection<EntryT[]>, curr: string) => {
    ...
    addEntries(entries: EntryT[]): void {
        entries.forEach((entry) => {
            ...
            Object.keys(this.dimensions).forEach((dimension) => {
                ...
                if (
                    dimensionMap.get(entryValueForDimension) === null ||
                    !dimensionMap.has(entryValueForDimension)
                ) {
    ...
    getEntriesInRange(
        dimension: keyof EntryT,
        start: string | number | null,
        end: string | number | null
    ): EntryT[] {
        if (!Object.prototype.hasOwnProperty.call(this.dimensions, dimension))
            throw new Error(`Dimension "${String(dimension)}" does not exist`)
        const targetDimension: OrderedMap<string | number, EntryT[]> =
            this.dimensions[dimension as string]
        const startIdx = start == null ? 0 : targetDimension.indexOf(start)
        const endIdx =
            end == null
                ? targetDimension.length - 1
                : targetDimension.indexOf(end)
        if (startIdx === -1)
            throw new Error(
                `Range start "${start}" does not exist in dimension "${String(
                    dimension
                )}"`
            )
        if (endIdx === -1)
            throw new Error(
                `Range end "${end}" does not exist in dimension "${String(
                    dimension
                )}"`
            )
    ...
    getSubsetArray(query: MatchQuery<string | number>): EntryT[] {
        const subsets = Object.entries(query).map(
            ([dimensionName, dimensionItem]) => {
                if (
                ...
                if (typeof dimensionItem === 'object') {
                    if (dimensionItem.range && dimensionItem.matches) {
                        ...
                        dimensionItem.matches.forEach((queryItem) => {
                            const matchResult = this.dimensions[
                                dimensionName
                            ].get(queryItem as number | string)
                            if (matchResult === undefined)
                                throw new Error(
                                    `"${queryItem}" does not exist in dimension "${dimensionName}"`
                                )
        ...
        return getArrayIntersection(
            ...subsets.filter((subset) => subset != null)
        )
    }

    getSubset(
        query: MatchQuery<string | number>,
        options: SubsetOptions = {}
    ): MultidimensionalMap<EntryT> {
        ...
        const subsetMap = new MultidimensionalMap<EntryT>(
        ...
        if (dimensions == null || dimensions.length === 0) {
            dataEntries.forEach((entry) => {
                _measures.forEach((measure) => {
                    if (output[measure as string] == null)
                        (output as Partial<EntryT>)[measure as string] = 0
                    ;(output as Partial<EntryT>)[measure as string] +=
                        entry[measure]
                })
            })
            return output
            ...
            _dimensions.forEach((dimension, idx) => {
                ...
                if (current[dimensionValue] == null) {
                    if (idx === lastItemIndex) {
                        ...
                        _measures.forEach(
                            (measure) =>
                                (subsetOfEntry[measure as string] =
                                    dataEntry[measure])
                        )
                        _dimensions.forEach(
                            (dimension) =>
                                (subsetOfEntry[dimension] =
                                    dataEntry[dimension])
                        )
                    ...
                    if (idx === lastItemIndex) {
                        _measures.forEach(
                            (measure) =>
                                (current[dimensionValue][measure] +=
                                    dataEntry[measure])
                        )
                    }
                }
