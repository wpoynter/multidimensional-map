...
test('Basic structure of the map', () => {
    ...
    expect(testMap.dimensions['day'].get('07/20/2020')).toStrictEqual(
        data.slice(0, 12)
    )
    expect(testMap.dimensions['hour'].get('9am')).toStrictEqual([
        data[3],
        data[4],
        data[5],
        data[15],
    ...
    expect(testMap.dimensions['item'].get('orange')).toStrictEqual([
        data[1],
        data[4],
        data[7],
        data[10],
...
describe('Subsetting features', () => {
    const testMap = new MultidimensionalMap(['day', 'hour', 'item'], data)
    test('Direct match', () => {
        expect(testMap.getSubsetArray({ hour: '9am' })).toStrictEqual([
            data[3],
            data[4],
            data[5],
            data[15],
        ...
        expect(testMap.getSubsetArray({ item: 'orange' })).toStrictEqual([
            data[1],
            data[4],
            data[7],
            data[10],
    ...
    test('Match queries', () => {
        expect(
            testMap.getSubsetArray({ item: { matches: ['orange', 'banana'] } })
        ).toStrictEqual(
            [1, 4, 7, 10, 13, 16, 19, 22, 2, 5, 8, 11, 14, 17, 20, 23].map(
                (i) => data[i]
            )
        )
    })
    test('Range queries', () => {
        expect(
            testMap.getSubsetArray({ hour: { range: ['9am', '11am'] } })
        ).toStrictEqual(
            [
        ...
        expect(
            testMap.getSubsetArray({ hour: { range: ['9am', null] } })
        ).toStrictEqual(
            [
                3, 4, 5, 15, 16, 17, 6, 7, 8, 18, 19, 20, 9, 10, 11, 21, 22, 23
            ].map((i) => data[i])
    ...
    test('Multi queries', () => {
        expect(
            testMap.getSubsetArray({
                day: '07/20/2020',
                item: 'banana',
                hour: '10am'
            })
        ).toStrictEqual([data[8]])
        expect(
            testMap
                .getSubset({ day: '07/20/2020' })
                .getSubset({ item: 'banana' })
                .getSubsetArray({ hour: '10am' })
        ).toStrictEqual([data[8]])
    })
    test('Error handling', () => {
        expect(() => testMap.getSubset({ day: { matches: ['foo'] } })).toThrow(
            /not exist/
        )
        expect(() =>
            testMap.getSubset({ hour: { range: ['9am', '13am'] } })
        ).toThrow(/not exist/)
    })
})
...
describe('Combining features', () => {
    const testMap = new MultidimensionalMap(['day', 'hour', 'item'], data)
    test('Error handling', () => {
        expect(() => testMap.combineEntries('numberSold', ['dday'])).toThrow(
            /does not exist/
        )
        expect(testMap.combineEntries('foo')).toEqual({ foo: NaN })
    })
    test('Total sum', () => {
        expect(testMap.combineEntries('numberSold')).toEqual({
            numberSold: 800
        })
    })
    test('One field', () => {
    ...
    test('Two fields', () => {
        expect(
            testMap
                .getSubset({ hour: '9am' })
                .combineEntries('numberSold', ['day', 'item'])
        ).toEqual({
            '07/20/2020': {
    ...
    test('Multiple Measures', () => {
        ...
        const miniTestMap = new MultidimensionalMap(
            ['day', 'hour', 'item'],
            miniData
        )
        expect(miniTestMap.combineEntries(['numberSold', 'measure2'])).toEqual({
            numberSold: 73,
            measure2: 6
        })
        expect(
            miniTestMap.combineEntries(['numberSold', 'measure2'], ['hour'])
        ).toEqual({
            '8am': { numberSold: 73, measure2: 6, hour: '8am' }
        })
        expect(
            miniTestMap.combineEntries(['numberSold', 'measure2'], ['item'])
        ).toEqual({
    ...
    test('Changing Order', () => {
        const wackyMap = new MultidimensionalMap(
            ['day', 'hour', 'item'],
            data,
            {
                day: ['07/21/2020', '07/20/2020'],
                hour: ['11am', '8am', '9am', '10am']
            }
        )
        expect(
            wackyMap.getSubsetArray({ hour: { range: ['9am', null] } })
        ).toEqual([
            { day: '07/20/2020', hour: '9am', item: 'apple', numberSold: 33 },
            { day: '07/20/2020', hour: '9am', item: 'orange', numberSold: 23 },
            { day: '07/20/2020', hour: '9am', item: 'banana', numberSold: 11 },
        ...
        expect(
            () =>
                new MultidimensionalMap(['day', 'hour', 'item'], data, {
                    day: ['07/21/2020', '07/20/2020'],
                    hours: ['11am', '8am', '9am', '10am']
                })
        ).toThrow()
        expect(
            new MultidimensionalMap(['day', 'hour', 'item'], data, {
                hour: ['11am', '10am']
            }).dimensions.hour.order
        ).toEqual(['11am', '10am', '8am', '9am'])
        expect(() =>
            new MultidimensionalMap(['day', 'hour', 'item'], data, {
                day: ['junk']
            }).combineEntries('numberSold', 'day')
        ).not.toThrow()
    })
    test('Incomplete Data', () => {
        const incompleteMap = new MultidimensionalMap(
            ['day', 'hour', 'item'],
            incompleteData
        )
        // 9am should normally be missing from the subsetted data
        expect(
            incompleteMap.getSubset({ day: '07/21/2020' }, { keepOrder: true })
                .dimensions.hour.order
        ).toEqual(['8am', '9am', '10am', '11am'])
        expect(
            incompleteMap
                .getSubset({ day: '07/21/2020' }, { keepOrder: true })
                .combineEntries('numberSold', 'hour')
        ).toEqual({
